...
public class cydBTL2CAPServer extends cydBTServer {
    protected void listen(String uuid, String name) {
        m_serverRunning = true;

        m_serverMessages.addElement("Starting server!");
        ...
        L2CAPConnectionNotifier l2capServer = null;
        
        try {
            l2capServer = (L2CAPConnectionNotifier)Connector.open("btl2cap://localhost:" + uuid + ";name=" + name);
            m_server = l2capServer;
        } catch (Throwable t) {
            if (m_showServerMessages)
                m_serverMessages.addElement("Server cannot start: " + t.getMessage());
        ...
        while (m_serverRunning) {
            ...
            try {
                m_serverMessages.addElement("Waiting for incoming connections");
                
                conn = l2capServer.acceptAndOpen();
